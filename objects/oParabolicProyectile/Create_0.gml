...
function ParabollicRightLeftMovement(){
	...
	if(!collision_rectangle(bbox_left,bbox_top,bbox_right,bbox_bottom,target,false,true)){
		...
		if (goingUp) && (point_distance(x,y,initHalfWayToTargetX,initHalfWayToTargetY) > 10 ){
			...
			//parabola (y = x^2) this will just make the proyectile go out of the screen
		...
		for(var i = 0; i < global.playerCharasOnBattle; i++){
			...
			if(createSpamMarker){
				...
				parentChara.CreateSpamMarker(oBattleManager.playerCharasOnBattleList[|i],0.4,hitMarkSpr,parentChara);
			}
			else {
	...
	if(!collision_rectangle(bbox_left,bbox_top,bbox_right,bbox_bottom,target,false,true)){
		...
		for(var i = 0; i < global.enemiesOnBattle; i++){
			...
			if(createSpamMarker){
				...
				parentChara.CreateSpamMarker(oBattleManager.enemiesOnBattleList[|i],0.4,hitMarkSpr,parentChara);
			}
			else {
